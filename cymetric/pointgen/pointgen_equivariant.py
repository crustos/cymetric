...
class EquivariantCICYPointGenerator(CICYPointGenerator):
    ...
    def generate_point_weights(self, n_pw, omega=False,
        ...
        Weights are computed on the base sample and tiled. That is not only
        cheaper, it is what is correct: an image ``g.p`` is drawn from the
        push-forward of the sampling measure, and because Gamma preserves
        ``|Omega|^2`` its importance weight is the weight of ``p``. (For a
        unitary Gamma the Fubini-Study mixture is invariant too and the two
        coincide; for a non-unitary one only the tiled weight is right.)

        ``omega`` is recomputed on the images rather than tiled. It is the
        value of Omega in the local chart the rest of the package picks at
        each point, and it is always used together with a metric determinant
        computed in that same chart, so it has to belong to the image's chart,
        not the original's. The two agree in modulus for monomial group
        matrices, but not in phase, and not at all in general.
        """
        ...
        # The parent drops points whose patch is ambiguous, so it can return
        # fewer than asked for; size the orbit blocks by what came back.
        n_got = len(base)
        pts = self.orbit(base['point'])
        dtype = list(base.dtype.descr) + [('orbit_index', np.int32)]
        out = np.zeros(len(pts), dtype=dtype)
        out['point'] = pts
        # Tiled, not recomputed -- see the docstring for why this is the
        # correct importance weight and not merely an approximation of it.
        out['weight'] = np.tile(base['weight'], m)
        if omega:
            out['omega'] = self.holomorphic_volume_form(pts)
        out['orbit_index'] = np.repeat(np.arange(m, dtype=np.int32), n_got)
        return out
